...
def obbAndSaving(readF, writeF, boxName):
    print(f"{boxName} is processing")
    pcd = o3d.io.read_point_cloud(readF)  # Read data
    ...
    o3d.io.write_point_cloud(writeF, mergedPoints)
    print(f"{boxName} is saved succesfully as {writeF}")
...
if __name__ == "__main__":
    ...
    for i in range(1, 6):
        iFile = f"data/pca/box_{i}_noisy.pcd"
        oFile = f"output/box{i}.pcd"
        boxId = f"Box{i}"
        obbAndSaving(iFile, oFile, boxId)
    print("All boxes saved succesfully")
